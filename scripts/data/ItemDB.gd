...
static func make_starter_weapon() -> Dictionary:
	...
	var prefixes: Array = [RVItemAffixDB.materialize_affix(prefix_def, "prefix", 1, null, 1)]
	return RVItemRollSystem.build_item_from_parts(base, "Magic", 1, prefixes, [], 24, {}, [], [], "A simple starter wand with enough potential to teach crafting.")
...
static func generate_test_item(rng: RandomNumberGenerator, item_level: int, slot: String = "", rarity: String = "") -> Dictionary:
	return RVItemRollSystem.roll_item(rng, max(1, item_level), _normalize_test_slot(slot), rarity)
...
static func normalize_item(item: Dictionary) -> Dictionary:
	...
	if not result.has("forge_potential"):
		result["forge_potential"] = 0 if str(result.get("rarity", "Normal")) == "Unique" else 8
	if not result.has("max_forge_potential"):
		result["max_forge_potential"] = int(result.get("forge_potential", 0))
	var calculated_stats: Dictionary = RVItemAffixDB.aggregate_stats(Dictionary(result.get("implicit_stats", {})), Array(result.get("prefixes", [])), Array(result.get("suffixes", [])), Dictionary(result.get("extra_stats", {})))
...
static func rebuild_item(item: Dictionary) -> Dictionary:
	return normalize_item(RVItemRollSystem.rebuild_item(item))
...
static func _normalize_test_slot(slot: String) -> String:
	match slot:
