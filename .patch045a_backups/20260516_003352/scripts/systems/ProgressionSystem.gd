...
static func award_kill(state: RVGameState) -> void:
	...
	state.add_xp(12.0 + float(state.room_index) * 2.0) if str(state.current_activity.get("kind", "")) == "map": RVMapSystem.award_map_enemy_drop(state, max(1, state.level + state.room_index))
