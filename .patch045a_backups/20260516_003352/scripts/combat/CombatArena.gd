...
func spawn_room(state: RVGameState) -> void:
	...
	state.room_objective = "Defeat all enemies." if str(activity.get("kind", "")) == "map": _spawn_map_room(state) return
...
func _spawn_map_room(state: RVGameState) -> void:
	...
	state.room_reward_claimed = false
	state.room_exit_ready = false
	var map_item: Dictionary = activity.get("map", {})
	...
	var spawn_points: Array[Node] = _enemy_spawn_points()
	var enemy_mix: Array = map_item.get("enemy_mix", ["Grunt", "Archer", "Spitter", "Brute"])
	for index: int in range(count):
		var spawn_pos: Vector2 = Vector2(640.0, 360.0)
		if spawn_points.size() > 0:
			var marker: Node2D = spawn_points[index % spawn_points.size()]
