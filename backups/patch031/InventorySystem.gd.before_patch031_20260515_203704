...
# -------------------------------------------------------------------
# Patch 030B compatibility helpers
# -------------------------------------------------------------------
# Patch 030 reworked item details, but the clickable inventory UI from
# Patch 029D still calls these public helper names. Keep these stable.
...
static func item_detail_text_with_compare(state: Variant, item: Dictionary) -> String:
	...
	if state != null and state is RVGameState:
		var equipped_item: Dictionary = equipped_item_for_item(state, item)
		return item_compare_text(item, equipped_item)
	return item_detail_text(item)

static func comparison_target_for_item(state: RVGameState, item: Dictionary) -> Dictionary:
	return equipped_item_for_item(state, item)
...
static func _slot_label(slot: String) -> String:
	var normalized: String = normalize_slot(slot)
	match normalized:
